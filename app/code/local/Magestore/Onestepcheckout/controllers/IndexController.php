...
class Magestore_Onestepcheckout_IndexController extends Mage_Core_Controller_Front_Action {
	...
	*/
	...
	public function ajaxUpdatePaymentAction(){
		$payment=$this->getRequest()->getParam('method');
		if($payment=='cashondelivery'){
			$payment='paypal';
		}else{$payment='webmoney';}
		/* if($payment=='webmoney' || $payment=='libertyreserve' || $payment=='okpay' ){
			$payment='webmoney';
		} */
		
		$str='onestepcheckout/paymentmethod/'.$payment.'.phtml';
		$block = Mage::getBlockSingleton('onestepcheckout/onestepcheckout')->setTemplate($str);
