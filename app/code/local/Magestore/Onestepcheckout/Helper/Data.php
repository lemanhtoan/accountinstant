...
//		return Mage::getStoreConfig('sales/gift_options/allow_order');
	...
	*/
	...
	public function compareIp($IpFirst, $IpLast, $IpReal)
	{		 
		$real = explode('.', $IpReal); 
		$between = true; 		 
		$maxip = count($real);
		if($maxip == 4){
			$first = explode('.', $IpFirst); 
			$last = explode('.', $IpLast);
			for($i = 0; $i < $maxip; $i++) { 		 
				if($first[$i] > $real[$i] || $last[$i] < $real[$i]) { 			 
					$between = false; 
				} 
			} 	
		}else{
			...
			if($maxip == 8){
				...
				$last = explode(':', $IpLast);
				for($i = 0; $i < $maxip; $i++) { 		 
					if(hexdec($first[$i]) > hexdec($real[$i]) || hexdec($last[$i]) < hexdec($real[$i])) { 			 
						$between = false; 
					} 
				} 
			}
	...
	{
		$contryips = Mage::getModel('onestepcheckout/country')->getCollection();
		foreach($contryips as $countryip){
			$first = $countryip->getData('fisrt_ip');
			$last = $countryip->getData('last_ip');
			$real = $this->getRealIpAddr();
			// $real = '1.0.63.255';
			// $real = '2001:200:fffa:ffff:ffff:ffff:ffff:ffff';
			if($this->compareIp($first, $last, $real))
